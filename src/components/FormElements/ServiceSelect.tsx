...
const ServiceSelect: React.FC<DropdownProps> = ({ id, onChange, disable = false }) => {
  ...
  const open = () => {
    setShow(true); // Ne pas ouvrir si le formulaire est soumis
  };
  ...
  const select = (index: number, event: React.MouseEvent) => {
    ...
    onChange(newSelected.map((optionIndex) => newOptions[optionIndex].value));
  };
  ...
  useEffect(() => {
    // Réinitialiser la sélection si disable est activé
    if (disable) {
      setSelected([]);
    ...
    <div className="relative z-50">
      ...
      <div>
        <select className="hidden" id={id}>
          ...
          <option value="Traveling Clinic">Traveling Clinic</option>
          <option value="Power supply & Accessories">Power supply & Accessories</option>
                  ...
                  <div className="flex flex-auto flex-wrap gap-3">
                    ...
                    {selected.length === 0 && (
                      <div className="flex-1">
                        <input
                          placeholder="Select services that the profile can access"
